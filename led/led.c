 ...
 * 打开设备
 * 传递给驱动的inode
 * 设备文件file 结构体有个叫private_data 
 * 的成员变量一般在open的时候将private_data指向设备结构体
 * 0成功;其它失败
 */ 
...
static ssize_t led_write(struct file *filp,const char __user *buf,size_t cnt, loff_t *offt)
{
    ...
    if(ledstat == LEDON)
    {
        led_switch(LEDON);          /*打开LED 灯*/
    }else if(ledstat == LEDOFF){
        led_switch(LEDOFF);         /*关闭LED 灯*/
    }
...
static int __init led_init(void)
{
    ...
    val &= ~(3<<26);    /*清除以前的设置*/
    val |= (3 <<26);    /*设置新值*/
    write(val,IMX6U_CCM_CCGR1);

    /*3.设置GPIO1_IO3的复用功能，将其复用为GPIO1_IO03,最后设置IO属性*/
